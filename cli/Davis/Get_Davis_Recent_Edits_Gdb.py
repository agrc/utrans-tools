...
# Set local variables
updateFeatures = r"L:\agrc\data\county_obtained\Davis\DavisCounty_20260626.gdb\DavisRoads" ### THIS WOULD BE THE NEWEST DATA
baseFeatures = r"L:\agrc\data\county_obtained\Davis\DavisCounty_20260514.gdb\DavisRoads" ### THIS IS THE DATA THEY SENT US LAST TIME
...
for item in list:
    ...
    for row in rows:
        if row.PrefixDirection == ' ' or row.PrefixDirection == None or row.PrefixDirection is None:
            row.PrefixDirection = ""
        if row.RoadName == ' ' or row.RoadName == None or row.RoadName is None:
            row.RoadName = ""
        if row.RoadNameType == ' ' or row.RoadNameType == None or row.RoadNameType is None:
            row.RoadNameType = ""
        if row.PostDirection == ' ' or row.PostDirection == None or row.PostDirection is None:
            row.PostDirection = ""
        if row.RoadAliasName == ' ' or row.RoadAliasName == None or row.RoadAliasName is None:
            row.RoadAliasName = ""
...
## compare values
# compare_fields = "PrefixDirection PrefixDirection; RoadName RoadName; RoadNameType RoadNameType; PostDirection PostDirection; RoadAliasName RoadAliasName; LeftFrom LeftFrom; LeftTo LeftTo; RightFrom RightFrom; RightTo RightTo"
compare_fields = "PrefixDirection PrefixDirection; RoadName RoadName; RoadNameType RoadNameType; PostDirection PostDirection; RoadAliasName RoadAliasName; LeftFrom LeftFrom; LeftTo LeftTo; RightFrom RightFrom; RightTo RightTo; MPH MPH"
