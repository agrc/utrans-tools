...
#env.workspace = r"D:\UTRANS\Updates\CacheCenterlines_16_02_17.gdb" ### change database name ###

#strTimeNow = time.strftime("%c")

# Set local variables
updateFeatures = r"L:\agrc\data\county_obtained\Cache\CacheCo_20251117.gdb\CacheCountyRoads" ### THIS WOULD BE THE NEWEST DATA
baseFeatures = r"L:\agrc\data\county_obtained\Cache\CacheCo_20250515.gdb\Roads" ### THIS IS THE DATA THEY SENT US LAST TIME
...
dfcOutput = dirname + "\\DFC_CacheToCache"
...
for item in list:
    ...
    for row in rows:
        if row.PRE_DIR == ' ' or row.PRE_DIR == None or row.PRE_DIR is None:
            row.PRE_DIR = ""
        if row.GEOCODEST == ' ' or row.GEOCODEST == None or row.GEOCODEST is None:
            row.GEOCODEST = ""
        if row.TYPE == ' ' or row.TYPE == None or row.TYPE is None:
            row.TYPE = ""
        if row.SUFFDIR == ' ' or row.SUFFDIR == None or row.SUFFDIR is None:
            row.SUFFDIR = ""
        if row.AliasPreDir == ' ' or row.AliasPreDir == None or row.AliasPreDir is None:
            row.AliasPreDir = ""
        if row.AliasStreet == ' ' or row.AliasStreet == None or row.AliasStreet is None:
            row.AliasStreet = ""
        if row.AliasType == ' ' or row.AliasType == None or row.AliasType is None:
            row.AliasType = ""
        if row.AliasSufDir == ' ' or row.AliasSufDir == None or row.AliasSufDir is None:
            row.AliasSufDir = ""
        if row.FAL == ' ' or row.FAL == None or row.FAL is None:
            row.FAL = 0
        if row.TAL == ' ' or row.TAL == None or row.TAL is None:
            row.TAL = 0
        if row.FAR == ' ' or row.FAR == None or row.FAR is None:
            row.FAR = 0
        if row.TAR == ' ' or row.TAR == None or row.TAR is None:
            row.TAR = 0
...
#match values
match_fields = "GEOCODEST GEOCODEST"
...
## compare values
compare_fields = "PRE_DIR PRE_DIR; GEOCODEST GEOCODEST; TYPE TYPE; FAL FAL; TAL TAL; FAR FAR; TAR TAR; AliasPreDir AliasPreDir; AliasStreet AliasStreet; AliasType AliasType; AliasSufDir AliasSufDir; SUFFDIR SUFFDIR"

arcpy.AddMessage("Begining detect feature change process for Cache at: " + time.strftime("%c"))
...
expression = r"DFC_CacheToCache.CHANGE_TYPE <> 'NC'"
layerName = "roads_lyr"
