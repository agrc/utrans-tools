...
# Set local variables
updateFeatures = r"L:\agrc\data\county_obtained\Cache\CacheCo_20260514.gdb\CacheCountyRoads" ### THIS WOULD BE THE NEWEST DATA
baseFeatures = r"L:\agrc\data\county_obtained\Cache\CacheCo_20260210.gdb\CacheCountyRoads" ### THIS IS THE DATA THEY SENT US LAST TIME
...
for item in list:
    ...
    for row in rows:
        ...
        if row.SUFFDIR == ' ' or row.SUFFDIR == None or row.SUFFDIR is None:
            row.SUFFDIR = ""
        if row.AliasPreDi == ' ' or row.AliasPreDi == None or row.AliasPreDi is None:
            row.AliasPreDi = ""
        if row.AliasStree == ' ' or row.AliasStree == None or row.AliasStree is None:
            row.AliasStree = ""
        if row.AliasType == ' ' or row.AliasType == None or row.AliasType is None:
            row.AliasType = ""
        if row.AliasSufDi == ' ' or row.AliasSufDi == None or row.AliasSufDi is None:
            row.AliasSufDi = ""
...
## compare values
compare_fields = "PRE_DIR PRE_DIR; GEOCODEST GEOCODEST; TYPE TYPE; FAL FAL; TAL TAL; FAR FAR; TAR TAR; AliasPreDi AliasPreDi; AliasStree AliasStree; AliasType AliasType; AliasSufDi AliasSufDi; SUFFDIR SUFFDIR"
